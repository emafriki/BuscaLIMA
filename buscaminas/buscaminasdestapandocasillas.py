...
def slot_pulsado(slot):
    ...
    # Destapar automáticamente los botones que no contienen minas y están en la posición 0
        if slot == 0 and slot not in bombas:
            for i in range(9):
                for j in range(9):
                    index = i * 9 + j
                    if index not in bombas:
                        slot_pulsado(index)
...
def elegir_nivel():
    nivel = messagebox.askquestion("Selecciona el nivel", "¿Qué nivel deseas jugar?",
                                   icon='question', type='yesno')
    if nivel == 'yes':
        iniciar_juego("facil")
    else:
        nivel = messagebox.askquestion("Selecciona el nivel", "¿Qué nivel deseas jugar?\nPresiona 'No' para Intermedio y 'Cancelar' para Difícil",
                                       icon='question', type='yesno')
        if nivel == 'yes':
            iniciar_juego("intermedio")
        elif nivel == 'no':
            iniciar_juego("dificil")

def iniciar_juego(nivel):
    global bombas, lista_botones, contador_banderas, boton_bandera, bombas_cercanas, win, reset, numero_pulsaciones, imagen_bomba, inicio, bandera, tiempo_habilitado, tomar_tiempo_fin, contador_tiempo, tiempo_fin, tiempo2, tiempo_inicio, banderas_disponibles, y

    if nivel == "facil":
        bombas = [random.randrange(25) for _ in range(5)]
    elif nivel == "intermedio":
        bombas = [random.randrange(50) for _ in range(10)]
    elif nivel == "dificil":
        bombas = [random.randrange(81) for _ in range(20)]

    print(f"Las ubicaciones de las bombas ({nivel}) son:", bombas)
    ...
    tiempo_inicio = time.time()
    tiempo()
    banderas_disponibles = min(10, len(bombas))
    contador_banderas.config(text="Banderas disponibles: " + str(banderas_disponibles))
    boton_bandera.config(state="normal")  # Habilitar el botón de bandera si estaba deshabilitado
...
# Añadir el botón de "Iniciar Juego"

boton_iniciar = Button(frame, text="Iniciar Juego", command=elegir_nivel)
boton_iniciar.grid(column=4, row=0, columnspan=2)
